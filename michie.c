...
void tail(int fd, char *name, int userInput, int mode, int reg)
{
            ...
            for (i = 0; i <= m; i++)
            {
                ...
                if (reg == 0)
                {
                    if (lines - userInput <= l)
                    {
                        ...
                        else if (lines - userInput == l)
                            l++; //for some reason a new line character is printing prematurely. just ommitting
                        else
                        {
                            printf(1, "\n");
                            l++;
            ...
            for (i = 0; i <= m; i++)
            {
                ...
                if (reg == 0)
                {
                    if (bytes - userInput <= tempbytes)
                    {
                        ...
                        else if (bytes - userInput == tempbytes)
                            tempbytes++; //for some reason a new line character is printing prematurely. just ommitting
                        else
                        {
                            printf(1, "\n");
                            tempbytes++;
    ...
    close(temp);
    if (n < 0)
    { //we have an error so show error message
...
int main(int argc, char *argv[])
{
    ...
    { // if we have 2 arguments then we call with default
        for (i = 1; i < argc; i++)
        {
